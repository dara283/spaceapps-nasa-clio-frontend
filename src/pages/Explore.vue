...
async function saveCurrent(){
  ...
  // ensure thresholds exist for all selected
  ...
  // Try backend first if we have a token; fall back to LocalStorage
  try {
    if (state.token) {
      ...
      alert('Saved to your dashboard (server).')
      return
    }
    throw new Error('no token')
  } catch {
    saveItem(state.user.email, payload)
    alert('Saved to your dashboard (local).')
  }
}
...
async function submitForm(e){
  e.preventDefault()
  if (!timeframe.value) { alert('Pick a Time Analysis option.'); return }
  if (selectedVariables.value.length === 0) { alert('Select at least one variable.'); return }
  ensureThresholdsForSelected()
